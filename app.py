...
try:
    ...
    # Per-column numeric ranges from the raw CSV (before fillna) for UI hints
    _ranges = []
    for col in columns:
        ser = pd.to_numeric(test_df[col], errors='coerce').dropna()
        nuni = int(ser.nunique())
        uni = sorted(ser.unique().tolist())
        uniques_out = [_json_safe_scalar(u) for u in uni] if nuni <= 24 else None
        _ranges.append({
            "name": col,
            "min": _json_safe_scalar(ser.min()),
            "max": _json_safe_scalar(ser.max()),
            "n_unique": nuni,
            "uniques": uniques_out,
        })
    FEATURE_RANGE_STATS = _ranges
...
def get_feature_ranges():
    """Min/max and distinct codes per model feature, derived from data/Dropout (1).csv."""
    if FEATURE_RANGE_STATS is None:
        return jsonify({"error": "Dataset not loaded; cannot compute feature ranges."}), 503
    return jsonify(FEATURE_RANGE_STATS)
@app.route('/predict', methods=['POST'])
def predict():
    try:
        data_json = request.get_json()
